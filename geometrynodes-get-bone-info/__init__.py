...
def Add_to_Node_Menu(self, context):
    if context.area.type == "NODE_EDITOR" and context.space_data.tree_type == "GeometryNodeTree":
        layout = self.layout
        layout.popover("SNA_PT_GEO_NODE_PANEL", text="bone info", icon_value=0)


class SNA_PT_GEO_NODE_PANEL(bpy.types.Panel):
    bl_label = "New Panel"
    bl_idname = "SNA_PT_GEO_NODE_PANEL"
    ...
    def execute(self, context):
        ...
        if context.area.type == "NODE_EDITOR" and context.space_data.tree_type == "GeometryNodeTree":
            ...
            for group_temp in node_groups:
                if group_temp.bl_idname == "GeometryNodeTree" and group_temp.name == active_bone.name + "Info":
